...
def keywords_db():
		if (request.query.key):
			searchkey2 = request.query.key
		else:
			searchkey2 = searchkey
		print 'in keywords_db(), key= '+searchkey2
		global numrows
		global numpages
		print 'access_token: '+access_token
		if access_token == 'signed out' or access_token == 'error' or access_token == 'undefined':
		...
		try:	
			...
			c.execute('''INSERT INTO Keywords VALUES(?, 1) ''', [searchkey2])
			...
			b.execute('''SELECT docIndex.url
						...
						WHERE lex.word = ? 
						ORDER BY rank DESC, url ASC''', [searchkey2])
			rows = b.fetchall()

			for row in rows: 
				c.execute('''INSERT INTO Data VALUES (?,?)''', [searchkey2, row[0]])

		except sqlite3.IntegrityError, m:	#existing keywords
			c.execute('''UPDATE Keywords SET totalcount = totalcount + 1 WHERE keyword = ?''', [searchkey2])
			c.execute('''SELECT url
						FROM Data
						WHERE keyword = ? ''', [searchkey2])
			rows = c.fetchall()
